...
function buildStructuredFilterCatalog() {
    ...
    allModelOptions = [...new Set(appData.inventory.map((item) => item.modelName ?? "Unknown"))].sort((a, b) => collator.compare(a, b));
...
function describeActiveState(sortValues, tokenGroups, maxPrice, maxMileage) {
    ...
    if (elements.brand.value !== "All")
        parts.push(`brand: ${elements.brand.value}`);
    if (elements.model.value !== "All")
        parts.push(`model: ${elements.model.value}`);
    if (elements.location.value !== "All")
        parts.push(`location: ${elements.location.value}`);
    if (tokenGroups.length)
        parts.push(`search: ${elements.search.value.trim()}`);
    if (maxPrice !== null && maxPrice !== DEFAULT_MAX_PRICE)
        parts.push(`max price: ${Math.round(maxPrice)}`);
    if (maxMileage !== null)
        parts.push(`max mileage: ${Math.round(maxMileage)}`);
    if (elements.fuel.value !== "All")
        parts.push(`fuel: ${elements.fuel.value}`);
    if (elements.seller.value !== "All")
        parts.push(`seller: ${elements.seller.value}`);
    if (elements.body.value !== "All")
        parts.push(`body: ${elements.body.value}`);
    if (elements.risk.value !== "All")
        parts.push(`risk: ${elements.risk.value}`);
    if (elements.unrated.value === "include")
        parts.push("including unrated");
